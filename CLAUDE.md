...
- "how they fit together" on the wall flies the same sixteen tiles into a data-flow layout
  ...
  rather than tidy, and leave a tile unconnected before inventing a lane for it. `SURFACES` and `FEEDS`
  in the same file are the other half — a fourth column of the things he actually gets (the
  06:30 message, a question on WhatsApp, a page, when he asks) and which tool comes out where,
  audited from the sending side in that doc's "Reaches Kahran" section.
